...
function spawnGiftImageAnimation(ev, imageSrc){
    ...
    // of many particles - but capped so it never grows bigger than
    // the digit timer box (115px).
    const size = Math.min(115, Math.round(lerp(70, 190, intensity)));
...
function spawnSuperchatAnimation(ev){
    ...
    // Keep tier-based width, but never let it exceed the digit
    // timer box (115px). Note: since every current tier (120-260)
    // is already above 115, all tiers will render at 115px here -
    // only the gift images (which start at 70px) still show a
    // visible size difference between tiers.
    const width = Math.min(115, SUPERCHAT_TIER_WIDTH[tier]);
