...
#     "color": "rgb(...)" or "#RRGGBB" (YouTube's OWN real background
#              color for THIS Super Chat, captured by the browser
#              extension's youtube.js and matched up here by amount -
#              see /superchat/color-update below - or None if no
#              color has arrived, either because the extension isn't
#              running or it just hasn't matched yet)
# }
recent_events = []
next_event_id = 1


# ---------------- SUPER CHAT REAL-COLOR MATCHING ----------------
# SSN's webhook (adds time + creates the event) and the browser
# extension's youtube.js (reads YouTube's own real per-message color
# straight off the DOM) are two completely independent pipelines with
# no shared ID between them - either one can arrive first. So:
#
#   - If SSN's event is already sitting in recent_events when the
#     color shows up, attach_color_to_recent_superchat() finds it by
#     matching USD amount and tags it directly.
#   - If the color shows up FIRST, it's stashed in
#     pending_superchat_colors[] and push_event() checks there (via
#     take_pending_superchat_color()) the moment the SSN event itself
#     gets created.
#
# Either way, a brand-new stray color with nothing to match (extension
# running but SSN never reported that donation) just expires unused
# after SUPERCHAT_COLOR_MATCH_WINDOW_SECONDS.
SUPERCHAT_COLOR_MATCH_WINDOW_SECONDS = 8

pending_superchat_colors = []  # [{"amount_usd", "color", "author", "ts"}, ...]


def _amounts_close(a, b):
    """True if two USD amounts are close enough to call the same
    donation, allowing a little slack for independent currency-
    conversion rounding between SSN's pipeline and the extension's."""
...
def attach_color_to_recent_superchat(usd_amount, color):
    """Find the most recent, still-uncolored superchat event matching
    usd_amount within the match window and tag it with color.

    Returns True if a match was found and updated, False otherwise.
    Caller already holds `lock` (RLock, so this is safe to call from
    inside push_event() too).
    """

    now = time.time()

    for event in reversed(recent_events):
        if event.get("type") != "superchat":
            continue
        if event.get("color"):
            continue
        if now - event.get("ts", 0) > SUPERCHAT_COLOR_MATCH_WINDOW_SECONDS:
            continue
        if _amounts_close(event.get("value") or 0.0, usd_amount):
            event["color"] = color
            return True
...
def take_pending_superchat_color(usd_amount):
    """Consume a pending color (posted by the extension before SSN's
    own event existed yet) matching usd_amount, if any. Returns the
    color string, or None. Caller already holds `lock`."""

    now = time.time()

    # Drop stale entries first so this list never grows unbounded.
    pending_superchat_colors[:] = [
        p for p in pending_superchat_colors
        if now - p["ts"] <= SUPERCHAT_COLOR_MATCH_WINDOW_SECONDS
    ]

    for i, pending in enumerate(pending_superchat_colors):
        if _amounts_close(pending["amount_usd"], usd_amount):
            return pending_superchat_colors.pop(i)["color"]

    return None


def usd_amount_from_text(amount_text):
    """Parse a raw amount string (e.g. '$5.00', 'PHP 100') the same
    way process_super_chat() does, returning a USD float or None."""

    parsed = parse_donation(amount_text)
...
def push_event(event_type, name, value, seconds, image_url=None, combo_count=1):
    ...
    with lock:
        ...
        if event_type == "superchat" and value:
            # A color may have already arrived from the browser
            # extension (youtube.js) before SSN's own event did -
            # claim it now if so. If not, attach_color_to_recent_
            # superchat() will find THIS event later when the color
            # does arrive (see /superchat/color-update).
            pending_color = take_pending_superchat_color(float(value))
            if pending_color:
                event["color"] = pending_color
...
def superchat_color_update():
    """Receive YouTube's OWN real background color for one Super Chat/
    Super Sticker from the browser extension (youtube.js reads it
    straight off the --yt-live-chat-paid-message-primary-color CSS
    custom property YouTube sets on that message's element).

    This never adds time or creates a donation event by itself - SSN's
    webhook is still the source of truth for that. This only supplies
    a color, matched to the right superchat event by USD amount (see
    attach_color_to_recent_superchat() / take_pending_superchat_color()
    above) so the overlay can animate with the REAL color instead of a
    fixed one.

    Expects JSON body: {"author": "...", "amountText": "$5.00", "color": "..."}
    """
    ...
    amount_text = str(data.get("amountText") or "").strip()
    color = str(data.get("color") or "").strip()

    if not amount_text or not color:
        return jsonify({
            "ok": False,
            "error": "amountText and color are required"
        }), 400

    usd_amount = usd_amount_from_text(amount_text)

    if usd_amount is None:
        return jsonify({
            "ok": False,
            "error": f"Could not parse amount: {amount_text!r}"
        }), 202

    with lock:
        matched = attach_color_to_recent_superchat(usd_amount, color)

        if not matched:
            # SSN's own event for this donation hasn't shown up yet -
            # stash the color so push_event() can claim it the moment
            # that event gets created (see take_pending_superchat_color()).
            pending_superchat_colors.append({
    ...
    print(
        f"SUPERCHAT COLOR [OVERLAY EXTENSION]: "
        f"{author or 'Unknown'} / {amount_text} (${usd_amount:.2f}) -> "
        f"{color} -> "
        f"{'matched existing event' if matched else 'stashed, waiting for SSN event'}"
    )
...
def run_test_superchat_full(usd, color, author):
    """
    Fires a simulated Super Chat through the SAME TWO independent
    paths a real one takes in production:

      1. SSN's webhook path - add_time() + push_event("superchat", ...),
         exactly what process_super_chat() does for a real donation.
      2. The browser extension's color path - the same matching logic
         /superchat/color-update uses (attach_color_to_recent_superchat /
         pending_superchat_colors), carrying YouTube's real per-message
         color.

    In production these two paths race - the color can land before OR
    after SSN's event, depending on network timing. This fires them in
    a RANDOM order with a small random delay each time, so repeated
    test fires exercise both orderings, not just the easy one. That
    makes this a real test of the matching logic itself, not just of
    the animation - useful since there's no real livestream handy to
    test against.
    """
    ...
    def fire_ssn_side():
        add_time(seconds)
        push_event("superchat", None, usd, seconds)
        print(f"TEST SUPERCHAT (full pipeline): SSN side fired -> ${usd:.2f}")

    def fire_color_side():
        with lock:
            matched = attach_color_to_recent_superchat(usd, color)
            if not matched:
                pending_superchat_colors.append({
                    "amount_usd": usd,
                    "color": color,
        ...
        print(
            f"TEST SUPERCHAT (full pipeline): color side fired -> "
            f"{color} ({'matched existing event' if matched else 'stashed, waiting for SSN side'})"
        )

    delay_a = random.uniform(0.05, 1.2)
    delay_b = random.uniform(0.05, 1.2)

    if random.random() < 0.5:
        threading.Timer(delay_a, fire_ssn_side).start()
        threading.Timer(delay_a + delay_b, fire_color_side).start()
    else:
        threading.Timer(delay_a, fire_color_side).start()
        threading.Timer(delay_a + delay_b, fire_ssn_side).start()


@app.route("/test/superchat-full")
def test_superchat_full():
    """
    Full round-trip test button: replicates exactly what a REAL Super
    Chat produces in production - SSN's donation event AND the browser
    extension's real YouTube color - without needing a real livestream
    with real gifters to test against. See run_test_superchat_full()
    for how the two sides are raced.
    """
    ...
    usd = request.args.get("usd", default=5, type=float)
    color = request.args.get("color", default="#F5C700", type=str)
    author = request.args.get("author", default="Test Viewer", type=str)

    threading.Thread(
        target=run_test_superchat_full,
        args=(usd, color, author),
        daemon=True
    ).start()

    return jsonify({"ok": True, "usd": usd, "color": color})
...
def test_panel():
    ...
    <body>
        ...
        <h2>Super Chat - REAL color (full pipeline test)</h2>
        <p class="hint">
            These replicate a REAL incoming Super Chat exactly:
            SSN's donation event AND the browser extension's real
            YouTube color, fired in a random order with a random
            delay each time - just like production. Useful when
            there's no live stream with real gifters handy to test
            against. Each tier button uses YouTube's actual color for
            that amount range.
        </p>
        <div class="row">
            <button class="sc-btn" style="background:#1565C0"
                onclick="fireSuperchatFull(1.50, '#1565C0', 'Blue')">🔵 $1.50 Blue</button>
            <button class="sc-btn" style="background:#00B8D4"
                onclick="fireSuperchatFull(3, '#00B8D4', 'Light blue')">🩵 $3 Light blue</button>
            <button class="sc-btn" style="background:#00A050"
                onclick="fireSuperchatFull(7, '#00C853', 'Green')">🟢 $7 Green</button>
            <button class="sc-btn" style="background:#C9A000; color:#000"
                onclick="fireSuperchatFull(15, '#FFCA28', 'Yellow')">🟡 $15 Yellow</button>
            <button class="sc-btn" style="background:#E65100"
                onclick="fireSuperchatFull(30, '#F57C00', 'Orange')">🟠 $30 Orange</button>
            <button class="sc-btn" style="background:#C2185B"
                onclick="fireSuperchatFull(70, '#E91E63', 'Magenta')">🟣 $70 Magenta</button>
            <button class="sc-btn" style="background:#B71C1C"
                onclick="fireSuperchatFull(150, '#E62117', 'Red')">🔴 $150 Red</button>
        </div>
        <div class="row" style="margin-top:10px; align-items:center;">
            <input type="number" id="customUsd" value="5" min="1" step="0.5"
                style="width:70px; padding:8px; border-radius:8px; border:none;">
            <input type="color" id="customColor" value="#00C853"
                style="width:48px; height:38px; border:none; border-radius:8px; padding:0; cursor:pointer;">
            <button class="sc-btn" onclick="fireSuperchatCustom()">🎨 Fire custom</button>
        </div>
    ...
    <script>
        ...
        async function fireSuperchatFull(usd, color, label){
            setStatus('Firing REAL Super Chat: $' + usd + ' (' + label + ', ' + color + ')' +
                       ' - SSN side and color side race in random order, watch the overlay...');
            await fetch('/test/superchat-full?usd=' + encodeURIComponent(usd) +
                        '&color=' + encodeURIComponent(color) +
                        '&author=' + encodeURIComponent(label + ' Test'));
        }

        async function fireSuperchatCustom(){
            const usd = parseFloat(document.getElementById('customUsd').value) || 5;
            const color = document.getElementById('customColor').value || '#00C853';
            setStatus('Firing REAL Super Chat: $' + usd + ' (' + color + ')...');
            await fetch('/test/superchat-full?usd=' + encodeURIComponent(usd) +
                        '&color=' + encodeURIComponent(color) +
                        '&author=' + encodeURIComponent('Custom Test'));
        }
...
/* Used when the browser extension supplied YouTube's OWN real color
   for this specific Super Chat (ev.color, via
   /superchat/color-update) - built dynamically instead of using a
   static image, so it always matches the actual message. Same
   "play button on a white plate" look already used for the GIFT
   banner in youtube.js, just recolored per-message. */
.superchat-color-badge {
    position: relative;
    box-sizing: border-box;
    border-radius: 10px;
    background: linear-gradient(
        135deg,
        color-mix(in srgb, var(--sc-color, #BF953F) 70%, black) 0%,
        color-mix(in srgb, var(--sc-color, #BF953F) 85%, white) 45%,
        var(--sc-color, #BF953F) 100%
    );
...
function applyTimerPop(big, giftName, colorOverride){
    const timeEl = document.getElementById('time');

    // Same animation as always (timeIncreasePop / timeIncreasePopHuge) -
    // only the two colors it flashes through change. colorOverride
    // (YouTube's own real Super Chat color, ev.color) takes priority
    // over a gift's own two-tone colors when present; otherwise fall
    // back to the gift lookup, then to green/gold for anything with
    // neither (numpad hotkeys, colorless Super Chats, etc).
    const colors = colorOverride
        ? [colorOverride, colorOverride]
        : getGiftPopColors(giftName);
...
// Builds a colored "SUPERCHAT" badge using YouTube's OWN real
// background color for THIS specific message (ev.color, captured by
// the browser extension's youtube.js off the
// --yt-live-chat-paid-message-primary-color CSS custom property and
// matched up server-side by amount - see /superchat/color-update).
// Same "play button on a white plate" look already used for the GIFT
// banner in youtube.js, just recolored per-message instead of a
// fixed image - so it always matches the actual Super Chat, whatever
// color/currency/region it came from.
function buildSuperchatColorBadge(color, width){
    const height = Math.round(width * 0.62);

    const badge = document.createElement('div');
    badge.className = 'superchat-color-badge';
    badge.style.width = width + 'px';
    badge.style.height = height + 'px';
    badge.style.setProperty('--sc-color', color);

    const plateSize = Math.round(height * 0.34);

    badge.innerHTML =
        '<div class="superchat-color-badge-label">SUPERCHAT</div>' +
        '<div class="superchat-color-badge-plate">' +
        '<svg viewBox="0 0 22 16" width="' + plateSize + '" height="' + Math.round(plateSize * 0.73) + '">' +
        '<polygon points="7,2 7,14 17,8" fill="' + color + '"/>' +
        '</svg>' +
        '</div>';
...
function spawnSuperchatAnimation(ev){
    ...
    // YouTube's own real color for THIS message, when the browser
    // extension managed to capture and match it in time. This takes
    // priority over any static image - a dynamically colored badge
    // that matches the actual message beats a fixed-color image.
    const realColor = ev.color || null;

    const fx = document.getElementById('fx-layer');
    const target = getFlyPositions();
    const startRot = Math.round((Math.random() - 0.5) * 20);

    // Single flying box - the ticket image/badge (or emoji) plus the
    // $ value both live inside it, so they move/fade as ONE unit.
    const wrap = document.createElement('div');
    wrap.className = 'superchat-wrap';
    wrap.style.width = width + 'px';
    wrap.style.setProperty('--glow', realColor || '#FFD700');

    if (realColor){
        wrap.style.height = Math.round(width * 0.62) + 'px';
        wrap.appendChild(buildSuperchatColorBadge(realColor, width));
    } else if (imageSrc){
        const img = document.createElement('img');
    ...
    const flyColor = realColor || '#FFD700';
...
function scheduleLanding(anim, giftName, colorOverride){
    if (!anim) return;
    ...
    setTimeout(() => {
        ...
        if (EXPAND_TIMER_ON_INCREASE){
            applyTimerPop(!!anim.big, giftName, colorOverride);
        }
    ...
    if (!firstStateLoad && d.events && d.events.length){
        for (const ev of d.events){
            ...
            scheduleLanding(
                ...
                ev.type === "superchat" ? ev.color : null
            );
        }
        handledByEvent = true;
    }
