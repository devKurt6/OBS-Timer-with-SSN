...
def combo_key(user_key, gift_name):
    """Normalize a (user, gift) pair into a case/whitespace/@-insensitive
    lookup key, so the SAME combo is recognized whether it comes from
    SSN's webhook (e.g. user_key="ParasocialwithDonBenitez", gift_name=
    "Star") or from the browser extension reading YouTube's own pinned
    combo-counter DOM element (which may report slightly different
    casing/formatting, e.g. "@ParasocialwithDonBenitez" / "star")."""

    norm_user = (user_key or "").strip().lstrip("@").lower()
    norm_gift = (gift_name or "").strip().lower()
