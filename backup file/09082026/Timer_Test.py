...
# By default Flask/Werkzeug prints a line for EVERY request, including
# the overlay's constant polling of /state - which drowns out the
# jewel gift debug prints below. This raises the logging threshold so
# only real errors get printed, not routine request lines.
import logging
...
# ---------------- SESSION GIFT COUNTERS (for debug/confirmation only) ----------------
# Tracks how many Jewel gifts have been received this run, and how many of
# those came with a real numeric value straight from SSN vs. our own
# hardcoded GIFT_JEWEL_VALUES fallback table. Purely for terminal logging -
# does not affect the timer itself.
total_gifts_received = 0
total_jewels_from_ssn = 0.0
gifts_from_ssn_count = 0
gifts_from_fallback_count = 0
...
def process_ssn_paid_event(data):
    ...
    if event_type == "jeweldonation":
        ...
        global total_gifts_received
        global total_jewels_from_ssn
        global gifts_from_ssn_count
        global gifts_from_fallback_count

        # Dump the ENTIRE raw payload SSN sent for this gift, exactly as
        # received, before any of our own parsing/fallback logic touches
        # it. This is for inspection only - to see every field SSN gives
        # us (in case there's useful info we're not using yet).
        print("\n==== RAW SSN JEWEL GIFT PAYLOAD ====")
        try:
            print(json.dumps(data, indent=2, default=str))
        except Exception as e:
            print(f"(could not json-dump payload: {e}) raw: {data!r}")
        ...
        # RAW value straight from SSN, before any fallback logic touches it.
        # This is printed on its own so you can confirm in the terminal
        # whether SSN is really sending a real jewel number, independent
        # of whatever get_gift_jewel_amount() ends up resolving to.
        raw_ssn_value = gift.get("jewelsAmount")
        ...
        with lock:
            ...
            if source == "SSN":
                gifts_from_ssn_count += 1
                total_jewels_from_ssn += jewels
            elif source == "gift-name fallback":
                gifts_from_fallback_count += 1
            gift_number_this_session = total_gifts_received

        print(
            f"\n--- JEWEL GIFT #{gift_number_this_session} RECEIVED ---\n"
            f"Gift name       : {gift_name}\n"
            f"Raw SSN value   : {raw_ssn_value!r} "
            f"(meta.youtubeGift.jewelsAmount, exactly as SSN sent it)\n"
            f"Resolved jewels : {jewels:g} (source: {source})\n"
            f"Session totals  : {total_gifts_received} gifts received "
            f"({gifts_from_ssn_count} from SSN, "
            f"{gifts_from_fallback_count} from hardcoded fallback)"
        )
...
def test_panel():
    ...
    <body>
        ...
        <p class="hint">
            Tapping these does NOT add real time - it only triggers
            the animation. Keep the OBS overlay (or another tab of
            this page's main URL) open to see it fire.
        </p>
...
function spawnGiftImageAnimation(ev, imageSrc){
    ...
    img.style.setProperty('--start-x', target.startX + 'px');
    img.style.setProperty('--start-y', target.startY + 'px');
    img.style.setProperty('--end-x', target.endX + 'px');
