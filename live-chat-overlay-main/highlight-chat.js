...
+'padding:20px;'
+'width:100%;'
+'transition:.5s all cubic-bezier(.25,.25,.105,1.2);'
...
+'">' + data.authorname
  ...
  setTimeout(restoreChatScrolling, 300);
  setTimeout(restoreChatScrolling, 700);
  setTimeout(restoreChatScrolling, 1200);
...
function restoreChatScrolling() {
    ...
    // Remove focus from YouTube moderation popup
    if (document.activeElement) {
        document.activeElement.blur();
    }

    // Refocus the chat scroller
    scroller.focus?.();
