...
if (data.sticker) {
    ...
    // donation amount) to recolor the WHOLE ribbon with a dark-light-dark
    // "shine" gradient (light band in the middle, like the original gold
    // ribbon's metallic sheen), instead of a flat color. Falls back to
    // the default CSS gold gradient when there's no tier color available.
    var scCardBg = data.tierColor
      ? 'background:linear-gradient(to bottom, color-mix(in srgb, '+data.tierColor+' 60%, black), color-mix(in srgb, '+data.tierColor+' 25%, white) 50%, color-mix(in srgb, '+data.tierColor+' 60%, black));'
      : '';
    var scTextColor = data.tierColor ? 'color:#111;' : '';

    data.donationHTML = `
    <div class="donation donation-icon-label" style="${scCardBg}">
        <div
            style="
                position:absolute;
                top:0;
        ...
        <div style="${scTextColor}">${data.donation}</div>
    </div>`;
