...
if (data.sticker) {
    ...
    // donation amount) to recolor the WHOLE ribbon: tier color at top and
    // bottom, pure white shine in the middle. Falls back to the default
    // CSS gold gradient when there's no tier color available.
    var scCardBg = data.tierColor
      ? 'background:linear-gradient(to bottom, '+data.tierColor+', #fff 50%, '+data.tierColor+');'
      : '';
