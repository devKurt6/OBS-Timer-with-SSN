...
if (data.sticker) {
    ...
    // donation amount) to recolor the WHOLE ribbon as a dark-to-light
    // gradient (same depth effect as the original gold ribbon), instead
    // of a flat color. Falls back to the default CSS gold gradient when
    // there's no tier color available.
    var scCardBg = data.tierColor
      ? 'background:linear-gradient(to bottom, color-mix(in srgb, '+data.tierColor+' 50%, black), '+data.tierColor+', color-mix(in srgb, '+data.tierColor+' 40%, white));'
      : '';
